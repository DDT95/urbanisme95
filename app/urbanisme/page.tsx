...
function buildPublicBuildingsCql(){
  const natures=new Set<string>(), usages=new Set<string>();
  Object.values(publicBuildingCategoryFilters).forEach((filter)=>{filter.natures?.forEach((n)=>natures.add(n));filter.usages?.forEach((u)=>usages.add(u));});
  const quote=(values:Set<string>)=>[...values].map((value)=>`'${value.replace(/'/g,"''")}'`).join(",");
  const clauses:string[]=[];
  if(natures.size)clauses.push(`nature IN (${quote(natures)})`);
  if(usages.size)clauses.push(`usage_1 IN (${quote(usages)}) OR usage_2 IN (${quote(usages)})`);
...
function elecConsumptionValue(entry:Record<string,unknown>):number|null{
  for(const [key,value] of Object.entries(entry||{})) if(typeof value==="number" && /elec/i.test(key) && /conso/i.test(key)) return value;
  return null;
}
function elecColor(value:number){ if(value>=200)return "#c1121f"; if(value>=120)return "#e1541f"; if(value>=80)return "#e3b341"; if(value>=40)return "#a8c936"; return "#3cb44a"; }
...
export default function UrbanismePage() {
  ...
  const publicBuildingsLayerRef = useRef<any>(null);
  const publicBuildingsDataRef = useRef<Map<string, any>>(new Map());
  const publicBuildingsLoadedRef = useRef(false);
  const publicBuildingsLoadingRef = useRef(false);
  const publicBuildingsRequestRef = useRef<AbortController | null>(null);
  const publicBuildingsEnrichRef = useRef<Map<string, { dpe?: string; destination?: string; elec?: number | null; dpeLoaded?: boolean; riskCount?: number; riskNames?: string; riskLoaded?: boolean }>>(new Map());
  const enrichmentInFlightRef = useRef<{ dpe: boolean; risks: boolean }>({ dpe: false, risks: false });
    ...
    const launch = () => {
      ...
      const refreshGpuLayers = async () => {
        ...
        try {
          ...
          const tooltipMode:"none"|"hover"=zoomNow>=13&&zoomNow<16?"hover":"none";
          const bindInfo=(layer:any,html:string,className:string)=>{
            if(tooltipMode==="hover")layer.bindTooltip(html,{sticky:true,className});
          };
          if(wantsZones){ const nextZones=L.geoJSON(zoneData,{style:(feature:any)=>({color:zoneColor(feature),weight:1.5,fillColor:zoneColor(feature),fillOpacity:.26}),onEachFeature:(feature:any,layer:any)=>bindInfo(layer,`<div class="simple-map-tooltip"><b>Zone ${escapeHtml(firstValue(feature.properties,["libelle","typezone","libelle_zone"],"GPU"))}</b><span>${escapeHtml(firstValue(feature.properties,["partition","nomfic"],"Document d’urbanisme"))}</span></div>`,"urban-map-tooltip")}).addTo(map);if(previousZones&&map.hasLayer(previousZones))map.removeLayer(previousZones);pluTilesRef.current=nextZones;if(pluOverviewLayerRef.current&&map.hasLayer(pluOverviewLayerRef.current))map.removeLayer(pluOverviewLayerRef.current); }
          if(wantsSup){ const nextSup=L.geoJSON(supData,{style:(feature:any)=>({color:supColor(feature),weight:2.5,fillColor:supColor(feature),fillOpacity:.10,dashArray:feature.geometry?.type?.includes("Polygon")?"7 5":undefined}),pointToLayer:(feature:any,latlng:any)=>L.circleMarker(latlng,{radius:6,color:supColor(feature),weight:2,fillColor:supColor(feature),fillOpacity:.65}),onEachFeature:(feature:any,layer:any)=>{const p=feature.properties||{},code=supCode(feature);bindInfo(layer,`<div class="sup-tooltip"><b>${escapeHtml(code)} · ${escapeHtml(supTitle(feature))}</b><span>${escapeHtml(supFamily(code))}</span><small>${escapeHtml(firstValue(p,["typeass"],"Assiette de servitude"))}${p.srcgeoass?` · Source géométrique : ${escapeHtml(p.srcgeoass)}`:""}</small><small>Identifiant : ${escapeHtml(firstValue(p,["idass"],"non renseigné"))}</small></div>`,"sup-map-tooltip");}}).addTo(map);if(previousSup&&map.hasLayer(previousSup))map.removeLayer(previousSup);supTilesRef.current=nextSup;if(supOverviewLayerRef.current&&map.hasLayer(supOverviewLayerRef.current))map.removeLayer(supOverviewLayerRef.current); }
          parcelTilesRef.current?.bringToFront(); buildingTilesRef.current?.bringToFront();
          const parts=[]; if(wantsZones)parts.push(`${zoneData.features.length} zone${zoneData.features.length>1?"s":""}`); if(wantsSup)parts.push(`${supData.features.length} servitude${supData.features.length>1?"s":""}`); const modeText=tooltipMode==="hover"?" Survolez une géométrie pour le détail ; cliquez sur une parcelle pour sa fiche.":zoomNow<13?" Zoomez au niveau 13 pour afficher les infobulles au survol.":" Cliquez sur une parcelle pour sa fiche."; setLayerFeedback(`${parts.join(" · ")} dans cette vue.${modeText}`);
      ...
      const publicBuildingColor = (feature:any) => {
        const id=String(feature.id||feature.properties?.cleabs||"");
        const category=feature.properties?._category as PublicBuildingCategory|null;
        const mode=buildingInfoModeRef.current;
        const info=publicBuildingsEnrichRef.current.get(id);
        if(mode==="dpe")return info?.dpe&&info.dpe!=="Non renseigné"?dpeColor(info.dpe):"#c9cdd6";
        if(mode==="elec")return info?.elec!=null?elecColor(info.elec):"#c9cdd6";
        if(mode==="risks")return typeof info?.riskCount==="number"?publicRiskColor(info.riskCount):"#c9cdd6";
        return category?publicBuildingCategoryInfo[category].color:"#687787";
      };
      const publicBuildingStyle = (feature:any) => {
        const category=feature.properties?._category as PublicBuildingCategory|null;
      ...
      const bindPublicBuildingTooltip = (feature:any, layer:any) => {
        const id=String(feature.id||feature.properties?.cleabs||"");
        layer.bindTooltip(()=>{
          const p=feature.properties||{}; const category=p._category as PublicBuildingCategory|null;
          const info=publicBuildingsEnrichRef.current.get(id);
          const rows=[`<b>${escapeHtml(category?publicBuildingCategoryInfo[category].label:"Bâtiment public")}</b>`,`<span>${escapeHtml(p.nature||p.usage_1||"Nature non renseignée")}</span>`];
          if(info?.destination)rows.push(`<small>Destination BDNB : ${escapeHtml(info.destination)}</small>`);
          rows.push(`<small>DPE : ${escapeHtml(info?.dpe||"Non chargé")}</small>`);
          rows.push(`<small>Conso. électrique : ${info?.elec!=null?`${formatNumber(info.elec)} kWh/an (estim. BDNB)`:"Non chargée"}</small>`);
          if(typeof info?.riskCount==="number")rows.push(`<small>${info.riskCount} risque${info.riskCount>1?"s":""} recensé${info.riskCount>1?"s":""}${info.riskNames?` : ${escapeHtml(info.riskNames)}`:""}</small>`);
          return `<div class="simple-map-tooltip">${rows.join("")}</div>`;
        },{sticky:true,className:"urban-map-tooltip"});
      };
      const ensureDpeElecEnrichment = async () => {
        if(enrichmentInFlightRef.current.dpe)return;
        const targets=[...publicBuildingsDataRef.current.entries()].filter(([id,feature])=>{const category=feature.properties?._category as PublicBuildingCategory|null;return category&&publicBuildingCatsRef.current[category]&&!publicBuildingsEnrichRef.current.get(id)?.dpeLoaded;});
        if(!targets.length)return;
        enrichmentInFlightRef.current.dpe=true;setLayerLoading((current)=>({...current,dpePublic:true}));
        const controller=new AbortController();
        let cursor=0,done=0;
        const worker=async()=>{
          while(cursor<targets.length){
            const [id,feature]=targets[cursor++];
            if(controller.signal.aborted)return;
            const [lon,lat]=geometryCenter(feature.geometry);
            let dpe="",destination="",elec:number|null=null;
            try{
              const parcelResponse=await fetch(`https://apicarto.ign.fr/api/cadastre/parcelle?geom=${pointGeometry(lon,lat)}`,{signal:controller.signal});
              const parcelData:FeatureCollection=parcelResponse.ok?await parcelResponse.json():emptyCollection;
              const parcelId=firstValue(parcelData.features?.[0]?.properties,["idu"],"");
              if(parcelId){
                const bdnbResponse=await fetch(`https://api.bdnb.io/v1/bdnb/donnees/batiment_groupe_complet/parcelle?parcelle_id=eq.${encodeURIComponent(parcelId)}`,{signal:controller.signal});
                const bdnbData=bdnbResponse.ok?await bdnbResponse.json():[];
                const entry=Array.isArray(bdnbData)?bdnbData[0]:null;
                if(entry){ dpe=entry.classe_bilan_dpe||entry.classe_conso_energie_arrete_2012||""; destination=entry.usage_niveau_1_txt||entry.usage_niveau_2_txt||entry.destination||""; elec=elecConsumptionValue(entry); }
              }
            }catch(error:any){ if(error?.name==="AbortError")return; }
            publicBuildingsEnrichRef.current.set(id,{...(publicBuildingsEnrichRef.current.get(id)||{}),dpe:dpe||"Non renseigné",destination,elec,dpeLoaded:true});
            done++;
            if(done%20===0||done===targets.length)setLayerFeedback(`DPE et consommation électrique : ${done}/${targets.length} bâtiments publics analysés…`);
          }
        };
        await Promise.all(Array.from({length:Math.min(5,targets.length)},()=>worker()));
        if(!controller.signal.aborted){ applyPublicBuildingsStyle(); setLayerFeedback(`DPE et consommation électrique disponibles pour ${targets.length.toLocaleString("fr-FR")} bâtiments publics.`); }
        enrichmentInFlightRef.current.dpe=false;setLayerLoading((current)=>({...current,dpePublic:false}));
      };
      const ensureRisksEnrichment = async () => {
        if(enrichmentInFlightRef.current.risks)return;
        const targets=[...publicBuildingsDataRef.current.entries()].filter(([id,feature])=>{const category=feature.properties?._category as PublicBuildingCategory|null;return category&&publicBuildingCatsRef.current[category]&&!publicBuildingsEnrichRef.current.get(id)?.riskLoaded;});
        if(!targets.length)return;
        enrichmentInFlightRef.current.risks=true;setLayerLoading((current)=>({...current,publicRisks:true}));
        const controller=new AbortController();
        let cursor=0,done=0;
        const worker=async()=>{
          while(cursor<targets.length){
            ...
            if(controller.signal.aborted)return;
            const [lon,lat]=geometryCenter(feature.geometry);
            let riskCount=0,riskNames="";
            try{
              const response=await fetch(`https://georisques.gouv.fr/api/v1/gaspar/risques?latlon=${lon},${lat}`,{signal:controller.signal});
              const data=response.ok?await response.json():{data:[]};
              const riskDetails=(data.data||[]).flatMap((entry:any)=>entry.risques_detail||[]);
              riskCount=riskDetails.length;
              riskNames=uniqueValues(riskDetails.map((risk:any)=>risk.libelle_risque_long||risk.libelle_risque_jo||risk.risque)).join(", ");
            }catch(error:any){ if(error?.name==="AbortError")return; }
            publicBuildingsEnrichRef.current.set(id,{...(publicBuildingsEnrichRef.current.get(id)||{}),riskCount,riskNames,riskLoaded:true});
            done++;
            if(done%20===0||done===targets.length)setLayerFeedback(`Risques Géorisques : ${done}/${targets.length} bâtiments publics analysés…`);
          }
        };
      ...
      const refreshPublicBuildings = async () => {
        const anyEnabled=Object.values(publicBuildingCatsRef.current).some(Boolean);
        if(!anyEnabled){
        ...
        if(publicBuildingsLoadedRef.current){
          applyPublicBuildingsStyle();
          if(!map.hasLayer(publicBuildingsLayerRef.current))publicBuildingsLayerRef.current.addTo(map);
        ...
        if(publicBuildingsLoadingRef.current)return;
        const bounds=departmentBoundsRef.current;
        if(!bounds){ setLayerFeedback("Préparation de l’emprise complète du Val-d’Oise…"); return; }
        publicBuildingsLoadingRef.current=true;setLayerLoading((current)=>({...current,publicBuildings:true}));
        ...
        try{
          ...
          collected.forEach((feature:any)=>{const category=publicBuildingCategoryOf(feature);if(!category)return;const id=String(feature.id||feature.properties?.cleabs||`${category}-${dataMap.size}`);dataMap.set(id,{...feature,properties:{...feature.properties,_category:category}});});
      ...
      map.on("moveend", () => { refreshBuildings(); refreshMos(); refreshPublicLand(); refreshGpuLayers(); refreshPublicBuildings(); });
  ...
  useEffect(() => {
    ...
    map.setMinZoom(10);
    map.fire("moveend");
  }, [publicBuildingCats, buildingInfoMode]);
  ...
  async function inspectPoint(lon: number, lat: number, address: string, addressMeta?: Partial<AddressResult>, addressLabel = "Adresse recherchée", parcelOverride?: FeatureCollection) {
    ...
    try {
      ...
      let mutations: any[] = [];
      const mutationRequest = parcelId ? mutationParams(parcelId) : null;
      if (mutationRequest) {
        try {
          const mutationResponse = await fetch(`https://app.dvf.etalab.gouv.fr/api/mutations3/${mutationRequest.commune}/${mutationRequest.sectionPrefixee}`);
          const mutationData = mutationResponse.ok ? await mutationResponse.json() : { mutations: [] };
          mutations = (mutationData.mutations || []).filter((mutation: any) => mutation.id_parcelle === parcelId);
        } catch { /* Le service DVF peut être momentanément indisponible. */ }
      }
      setResult({ address, addressLabel, commune: addressMeta?.city || firstValue(props, ["nom_com", "nom_commune"]), codeInsee: addressMeta?.citycode || firstValue(props, ["code_insee", "code_dep"], "—"), parcel, zones: zoneData.features || [], servitudes: supData.features || [], risks: riskDetails, buildings: Array.isArray(buildings) ? buildings : [], publicLand, mos: { mos2021: numberValue(mosProps.mos2021), mos2025: numberValue(mosProps.mos2025), surface: numberValue(mosProps["st_area(shape)"]) }, mutations }); setDetailsOpen(true);
  ...
  async function searchAddress(event: React.FormEvent) {
    ...
    const inseeMatch = query.trim().match(/^\d{5}$/);
    if (inseeMatch) {
      const feature = communes.find((item:any) => String(item.properties?.code) === inseeMatch[0]);
      if (!feature) { setMessage("Aucune commune du Val-d’Oise ne correspond à ce code INSEE."); return; }
      setCommuneCode(inseeMatch[0]); exploreCommune(inseeMatch[0]); setQuery("");
      return;
    }
  ...
  const loadingLabels=Object.entries(layerLoading).filter(([,waiting])=>waiting).map(([key])=>({buildings:"bâtiments",mos:"MOS 2025",plu:"zonage PLU",servitudes:"servitudes",publicLand:"foncier public",publicBuildings:"bâtiments publics",dpePublic:"DPE bâtiments publics",publicRisks:"risques bâtiments publics"}[key]));
  return (
    <main className="urban-tool">
      ...
      <div className="urban-layout">
        <aside className="urban-panel">
          ...
          <div className="commune-autocomplete"><label htmlFor="urban-commune">1 · Choisissez une commune <small>(par nom ou code INSEE — nécessaire pour chercher une parcelle par référence cadastrale)</small></label><div><input id="urban-commune" value={communeQuery} placeholder="Commencez à saisir : Pontoise ou 95500…" autoComplete="off" onFocus={()=>setCommuneSuggestionsOpen(true)} onChange={(event)=>{setCommuneQuery(event.target.value);setCommuneSuggestionsOpen(true);setCommuneCode("");}}/>{communeSuggestionsOpen && communeQuery.trim().length>0 && <div className="commune-suggestions">{communes.filter((item)=>{const needle=communeQuery.trim().toLocaleLowerCase("fr");const code=String(item.properties?.code||"");return String(item.properties?.nom||"").toLocaleLowerCase("fr").includes(needle)||code.includes(needle);}).slice(0,6).map((item)=><button key={item.properties?.code} type="button" onClick={()=>{setCommuneCode(item.properties.code);exploreCommune(item.properties.code);}}><strong>{item.properties?.nom}</strong><small>Val-d’Oise · {item.properties?.code}</small></button>)}</div>}</div>{activeCommune && <p><i/>Vous explorez <strong>{activeCommune}</strong><button type="button" onClick={resetSearch}>Quitter</button></p>}</div>
          ...
          <section className="urban-layer-panel urban-public-panel" aria-labelledby="urban-public-title">
            ...
            {Object.values(publicBuildingCats).some(Boolean) && <div className="public-buildings-mode"><strong>Que voir sur ces bâtiments ?</strong><div>
              ...
              <button type="button" className={buildingInfoMode==="elec"?"active":""} onClick={()=>setBuildingInfoMode("elec")}>Conso. électrique (BDNB)</button>
              <button type="button" className={buildingInfoMode==="risks"?"active":""} onClick={()=>setBuildingInfoMode("risks")}>Risques (Géorisques)</button>
            </div></div>}
            ...
            <p className="public-land-note"><i/>Catégorisation par nature/usage IGN BD TOPO ; destination précise, DPE et consommation électrique complétés depuis la BDNB.</p>
          </section>
      ...
      {result && detailsOpen && <aside className="observatory-drawer" aria-label="Détail de la parcelle"><div className="observatory-drawer-head"><div className="print-brand"><img src={`${basePath}/prefet-val-doise-logo.png`} alt="Préfet du Val-d’Oise"/><span><b>Fiche d’identité parcellaire</b><small>DDT du Val-d’Oise · {new Date().toLocaleDateString("fr-FR")}</small></span></div><div className="drawer-actions"><button className="print-parcel" onClick={openParcelPdf}>Consulter la fiche PDF</button><button onClick={closeParcelDetails} aria-label="Fermer et désélectionner la parcelle">×</button></div><small>{result.addressLabel} · {result.commune}</small><h2 className="drawer-address">{streetOnly(result.address)}</h2><div className="parcel-id-print">Parcelle {firstValue(parcelProps,["section"],"")} {firstValue(parcelProps,["numero"],"—")}</div></div><div className="observatory-drawer-body urban-results">
        ...
        <section><div className="result-heading"><h2>Occupation du sol — MOS 2025</h2></div>{result.mos?.mos2025 ? <><div className="mos-reading"><small>Usage observé en 2025</small><strong>{mosLabels[result.mos.mos2025] || `Poste MOS ${result.mos.mos2025}`}</strong><span>{result.mos.mos2021 === result.mos.mos2025 ? "Usage stable depuis 2021" : `Évolution depuis : ${mosLabels[result.mos?.mos2021 || 0] || `poste ${result.mos?.mos2021}`}`}</span></div><p className="source-caption">Source : Institut Paris Region, MOS 2021–2025, nomenclature détaillée à 79 postes.</p></> : <p className="empty-result">Occupation du sol non retournée à cet emplacement.</p>}</section>
        <section><div className="result-heading"><h2>Zonage d’urbanisme</h2></div>{result.zones.length ? result.zones.map((zone,index)=><div className="result-chip green" key={zone.id || index}><b>Zone {firstValue(zone.properties,["libelle","typezone","libelle_zone"],"GPU")}</b><small>{firstValue(zone.properties,["partition","nomfic"],"Document opposable")}</small></div>) : <p className="empty-result">Aucun zonage retourné par le GPU.</p>}{result.zones.length>0&&result.servitudes.length>0&&<p className="urbanism-cross-reading"><strong>À retenir</strong> Cette parcelle relève de {result.zones.map((zone)=>`la zone ${firstValue(zone.properties,["libelle","typezone","libelle_zone"],"GPU")}`).join(" et ")} et elle est également concernée par {result.servitudes.length} servitude{result.servitudes.length>1?"s":""}. Leur objet précis est détaillé dans le bloc suivant.</p>}</section>
        <section><div className="result-heading"><h2>Servitudes concernant la parcelle</h2></div><p className="result-count"><strong>{result.servitudes.length}</strong> assiette{result.servitudes.length>1?"s":""} intersectée{result.servitudes.length>1?"s":""} — liste complète retournée par le GPU</p>{result.servitudes.length?result.servitudes.map((sup,index)=>{const code=supCode(sup),p=sup.properties||{};return <div className="result-chip sup-detail" style={{borderLeftColor:supColor(sup)}} key={sup.id||`${code}-${index}`}><b>{code} · {supTitle(sup)}</b><span>{supDescription(code)}</span><small><strong>Famille :</strong> {supFamily(code)} · <strong>Emprise :</strong> {firstValue(p,["typeass"],"non renseignée")}</small><small><strong>Identifiant GPU :</strong> {firstValue(p,["idass"],"non renseigné")}{p.srcgeoass?` · Source géométrique : ${p.srcgeoass}`:""}</small>{p.fichier&&<small><strong>Acte associé :</strong> {p.fichier}</small>}</div>}) : <p className="empty-result">Aucune assiette surfacique, linéaire ou ponctuelle retournée pour cette parcelle.</p>}</section>
        <section><div className="result-heading"><h2>Risques recensés</h2></div>{result.risks.length ? <div className="risk-list">{result.risks.map((risk,index)=><span key={index}>{risk.libelle_risque_long || risk.libelle_risque || "Risque"}</span>)}</div> : <p className="empty-result">Aucun risque Gaspar retourné pour ce point.</p>}</section>
        <section><div className="result-heading"><h2>Historique des mutations</h2></div><p className="result-count"><strong>{result.mutations.length}</strong> mutation{result.mutations.length>1?"s":""} recensée{result.mutations.length>1?"s":""} depuis 2019 — DVF géolocalisées</p>{result.mutations.length?result.mutations.map((mutation,index)=><div className="result-chip mutation-detail" style={{borderLeftColor:mutationColor(mutation.nature_mutation)}} key={mutation.id_mutation||index}><b>{formatMutationDate(mutation.date_mutation)} · {mutation.nature_mutation||"Mutation"}</b><span>{formatEuro(mutation.valeur_fonciere)}</span><small>{[mutation.type_local,mutation.surface_reelle_bati?`${mutation.surface_reelle_bati} m² bâti`:null,mutation.nombre_pieces_principales?`${mutation.nombre_pieces_principales} pièce(s)`:null,mutation.surface_terrain?`${mutation.surface_terrain} m² de terrain`:null].filter(Boolean).join(" · ")||"Nature du bien non renseignée"}</small></div>) : <p className="empty-result">Aucune mutation retournée par les données DVF pour cette parcelle.</p>}<p className="source-caption">Source : DVF géolocalisées, DGFiP/Etalab. Les ventes antérieures à 2019, en Alsace-Moselle et à Mayotte ne sont pas couvertes.</p></section>
        <div className="urban-official-links"><strong>Vérifier auprès des services officiels</strong><a href="https://www.geoportail-urbanisme.gouv.fr/" target="_blank" rel="noreferrer">Géoportail de l’urbanisme ↗</a><a href="https://www.georisques.gouv.fr/" target="_blank" rel="noreferrer">Géorisques ↗</a><a href="https://www.cadastre.gouv.fr/" target="_blank" rel="noreferrer">Cadastre ↗</a><a href="https://app.dvf.etalab.gouv.fr/" target="_blank" rel="noreferrer">Demandes de valeurs foncières ↗</a></div>
