    ...
    const launch = () => {
      ...
      const refreshGpuLayers = async () => {
        ...
        try {
          ...
          if(wantsZones){ const nextZones=L.geoJSON(zoneData,{style:(feature:any)=>({color:zoneColor(feature),weight:1.5,fillColor:zoneColor(feature),fillOpacity:.26}),onEachFeature:(feature:any,layer:any)=>layer.bindTooltip(`<div class="simple-map-tooltip"><b>Zone ${escapeHtml(firstValue(feature.properties,["libelle","typezone","libelle_zone"],"GPU"))}</b><span>${escapeHtml(firstValue(feature.properties,["partition","nomfic"],"Document d’urbanisme"))}</span></div>`,{sticky:true,className:"urban-map-tooltip"})}).addTo(map);if(previousZones&&map.hasLayer(previousZones))map.removeLayer(previousZones);pluTilesRef.current=nextZones;if(pluOverviewLayerRef.current&&map.hasLayer(pluOverviewLayerRef.current))map.removeLayer(pluOverviewLayerRef.current); }
    ...
    <main className="urban-tool">
      ...
      <div className="urban-layout">
        ...
        <section className="urban-map-wrap">
          ...
          <div className={`map-guidance ${mapZoom >= 13 ? "ready" : ""}`}><strong>{mapZoom >= 13 ? "Explorez les données détaillées" : "Vue départementale"}</strong><span>{layerFeedback}</span></div>
