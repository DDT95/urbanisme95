    ...
    const launch = () => {
      ...
      const refreshGpuLayers = async () => {
        ...
        try {
          ...
          const tooltipMode:"none"|"hover"|"click"=zoomNow<13?"none":zoomNow<16?"hover":"click";
          const bindInfo=(layer:any,html:string,className:string)=>{
            if(tooltipMode==="none")return;
            if(tooltipMode==="hover"){layer.bindTooltip(html,{sticky:true,className});return;}
            layer.on("click",(event:any)=>{L.DomEvent.stopPropagation(event);L.popup({className}).setLatLng(event.latlng).setContent(html).openOn(map);});
          };
          ...
          const parts=[]; if(wantsZones)parts.push(`${zoneData.features.length} zone${zoneData.features.length>1?"s":""}`); if(wantsSup)parts.push(`${supData.features.length} servitude${supData.features.length>1?"s":""}`); const modeText=tooltipMode==="none"?" Zoomez au niveau 13 pour afficher les infobulles au survol.":tooltipMode==="hover"?" Survolez une géométrie pour le détail ; cliquez sur une parcelle pour sa fiche.":" Cliquez sur une géométrie pour son détail, ou sur une parcelle pour sa fiche."; setLayerFeedback(`${parts.join(" · ")} dans cette vue.${modeText}`);
